...
void comSave(ListaCom lstCom, Arbol& a)
{
	...
	if (contParametros(lstCom) == 3) {
		...
		{
			if (!Vacio(nomArch->info))
			{
				int op;
				do
				{
					printf("Advertencia: ya hay una secuencia guardada en el archivo ¿Desea sobreescribirla? (1- Si 0- No).");
					scanf("%i", &op);
				} while (op != 1 && op != 0);
				if (op == 0)
					printf("Resultado: el archivo no sera sobreescrito.");
				else
				{
